...
$post_data = [
    ...
    'action' => 'under_attack_on', // Action Jangan Diubah
...
// Ambil response utama
$main_response = json_decode($responses[0], true);

// Tampilkan hasil
if ($main_response && $main_response['status']) {
    $data = $main_response['data'];
    echo "Under Attack Mode berhasil diaktifkan!\n";
    echo "Pesan: " . $data['pesan'] . "\n";
    echo "Email: " . $data['email'] . "\n";
    echo "Detail:\n";
    echo "ID: " . $data['result']['id'] . "\n";
    echo "Status: " . $data['result']['value'] . "\n";
    echo "Terakhir Diubah: " . $data['result']['modified_on'] . "\n";
} else {
    echo "Gagal mengaktifkan Under Attack Mode!\n";
